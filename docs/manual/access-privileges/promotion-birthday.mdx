...
## Birthday user group {/* #birthday-user-group */}

1. Create a new [user group](groups-permissions.mdx) called `Birthday today` at **Groups & permissions > User groups > Add user group**.
...
## Birthday promotion {/* #birthday-promotion */}
...
The next time the *User group promotions* [cron job](../common-concepts/cron.mdx) runs, any users whose birthday is today will be added to the `Birthday today` group and will have *It's my birthday today* as a banner added underneath their name in any messages they have posted.