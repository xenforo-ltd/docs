...
The criteria that can be used is discussed in the [Criteria](../common-concepts/criteria.md) section. If no criteria is selected, the promotion will never be awarded automatically.
...
Promotions are governed by a routine [cron](../common-concepts/cron.md) task, which runs every hour.

If you need to process promotions immediately, you can do so by manually running the *User group promotions* [cron](../common-concepts/cron.md) entry at **Tools > Cron entries > User group promotions**
...
If you want to apply a group membership via [criteria](../common-concepts/criteria.md) to a selection of your entire registered user base regardless of whether or not they have been recently active, the correct way to do so is through [batch updates](../users/user-batch-update.md) rather than promotions.
...
Clicking the delete icon again will make the user eligible for that promotion once more and remove the user from the list, until the next time the [cron](../common-concepts/cron.md) task runs, assuming they still meet the criteria.
...
2. Alternatively, if you don't want to delete the user group but you want to remove all users from it, you may use [batch updates](../users/user-batch-update.md) to identify all members of a specific group, and then remove their membership. You can then delete the promotion definition.
3. If the promotion provides membership of a user group that has a wider purpose and may also be the target of other promotions, the best way to undo the promotions done by one specific promotion is to leave it active but change its [criteria](../common-concepts/criteria.md) such that it will not match any users. For example, you could change the criteria to match only users registered before January 1990, causing no users to match. As promoted users interact with the forum, their membership of the user group provided by the promotion will be removed.
