...
Depending upon how you installed XenForo, config.php may have created by copying the contents of the original `src/config.php.default` file, or the [XenForo installer](../installing-and-upgrading/install.mdx) may have created if for you.
...
## Full unicode / emoji support {/* #full-unicode--emoji-support */}
...
### Site-wide feature disable {/* #site-wide-feature-disable */}
...
#### enableMailQueue {/* #enablemailqueue */}
...
#### enableTemplateModificationCallbacks {/* #enabletemplatemodificationcallbacks */}
...
#### enableContentLength {/* #enablecontentlength */}
...
#### enableLivePayments {/* #enablelivepayments */}
...
#### enableClickjackingProtection {/* #enableclickjackingprotection */}
...
#### enableReverseTabnabbingProtection {/* #enablereversetabnabbingprotection */}
...
#### enableApi (2.1+) {/* #enableapi-21 */}

Disables access to the REST API, normally accessible via `<url>/api/`.

#### enableAddOnArchiveInstaller (2.1+) {/* #enableaddonarchiveinstaller-21 */}
...
#### enableOneClickUpgrade (2.1+) {/* #enableoneclickupgrade-21 */}
...
#### Example values {/* #example-values-1 */}
...
### Data and script locations {/* #data-and-script-locations */}
...
These settings control the behavior of the internal XenForo HTTP client, which is used to fetch resources from across the internet, such as images and web pages when using the [Image and link proxy](../configuration/proxy.mdx).
...
Defines the format for links using the [Image and link proxy](../configuration/proxy.mdx).
...
The settings for caching all fall within the `$config['cache']` section, and are discussed in more detail in the [Cache](cache.mdx) section of this manual.
...
### Page-level cache settings {/* #page-level-cache-settings */}

When a cache mechanism has been configured, it is possible to cache entire pages for guest users using the [pageCache](cache.mdx#guest-page-caching). This extremely powerful system can consume large quantities of cache resources, so it is disabled by default until resources are allocated to it. All details and options are described in the [Cache support > Guest page caching](cache.mdx#guest-page-caching) section of this manual.

- `$config['pageCache']['enabled']` = `false`;

## For developers and designers {/* #for-developers-and-designers */}
