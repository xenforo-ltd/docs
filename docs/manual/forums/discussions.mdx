...
Discussions are one of XenForo's [forum and thread types](forum-thread-types.mdx).
...
Discussion threads may reside in a dedicated [discussion forum](discussions.mdx#discussion-forums), or a [mixed-type forum](discussions.mdx#mixed-type-forums) that allows discussion threads.
...
By default, discussion forums contain [discussion](discussions.mdx#discussion-threads) threads and [poll](discussions.mdx#poll-threads) threads.
...
A mixed type forum is essentially a regular discussion forum that can also host other thread types. Support for [article](articles.mdx#article-threads) and [question](questions.mdx#question-threads) thread types may be added to discussion forums, which turns them into [mixed-type forums](discussions.mdx#mixed-type-forums).
...
Some thread types may not be contained in a mixed-type forum at all, [suggestions](suggestions.mdx) being one of those types.