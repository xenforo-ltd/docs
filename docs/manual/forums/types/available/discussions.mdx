...
Discussion threads may reside in a dedicated [discussion forum](#discussion-forums), or a [mixed-type forum](#mixed-type-forums) that allows discussion threads.
...
By default, discussion forums contain [discussion](#discussion-threads) threads and [poll](#poll-threads) threads.
...
#### Mixed type forums
A mixed type forum is essentially a regular discussion forum that can also host other thread types. Support for [article](articles.mdx#article-threads) and [question](questions.mdx#question-threads) thread types may be added to discussion forums, which turns them into [mixed-type forums](#mixed-type-forums).
