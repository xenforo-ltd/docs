...
## No need for a separate mobile app {/* #no-need-for-a-separate-mobile-app */}
...
## Features of the PWA {/* #features-of-the-pwa */}
...
Apple mobile devices running iOS 16.4 and newer can install the PWA using the _Add to Home Screen_ feature in Safari. This step is required in order to enable [push notifications](pwa.mdx#push-notifications) on iOS devices.
...
You must choose one of the [languages](../appearance/languages.mdx) installed on your forum to be the primary language for the progressive web app. Options are provided to allow you to make the selection.

#### Colors {/* #colors */}

While the styling of your progressive web app will be based on the settings of your [default style](../appearance/styles.mdx#setting-the-default-style), you may provide additional styling for the _Meta theme color_, which often colors the device interface itself, and the _Page background color_, upon which all content is placed. 
