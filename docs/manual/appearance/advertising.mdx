...
## Deploying an advert {/* #deploying-an-advert */}
...
### Display criteria {/* #display-criteria */}
...
This can be useful when used in conjunction with a [paid user upgrade](../users/user-upgrades.mdx) in which users may pay to view a version of your site with all ads hidden.

### Display order {/* #display-order */}

If you have multiple ads deployed to the same position, they will be displayed according to the [display order](../common-concepts/display-order.mdx) that you define.
...
## Advert positions {/* #advert-positions */}

The XenForo [templates](templates.mdx) are preconfigured with numerous locations in which ads can be placed with minimal effort. Within the template code, these appear as follows:
