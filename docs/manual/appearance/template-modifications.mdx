...
In the template modification manager, you will find a list of all template modifications currently in use, grouped by the [add-on](../configuration/add-ons.mdx) which has defined them. You may quickly disable and re-enable any template modification by using the toggle gadget in the list.
