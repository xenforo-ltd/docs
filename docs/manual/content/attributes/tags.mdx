---
sidebar_position: 10
---

# Tags

To help organise your forum, users may add tags to their content. These tags
become links, allowing topics that discuss similar things to be grouped and
searched.

### Tag manager

The tag manager lists all the tags that have been applied to content within your
forum, describing the number of items that have been tagged with each tag and
the date the tag was most recently applied.

From here, you may click to view all content tagged with a particular tag, or
you may merge tags if you decide that they are essentially the same thing.

You may also delete tags from the manager.
