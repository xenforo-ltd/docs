...
The Media Gallery follows the standard XenForo add-on packaging system and approaches. Full guidance for these actions can be found in the [add-ons section](add-ons.md).
...
Media will be placed into individual categories that are organized into a tree. In this regard, they are very similar to the [node tree](nodes-forums.md#the-node-tree).
...
Custom media fields allow you to define additional structured fields for users to fill in when creating a thread. These are similar to [custom thread fields](thread-fields.md).
