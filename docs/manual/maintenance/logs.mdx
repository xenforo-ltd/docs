...
#### Email bounce log {/* #email-bounce-log */}

If [configured](../configuration/email.mdx) to do so, XenForo will log attempts to send email to addresses that bounce the message back due to errors such as the address not existing, the mailbox being full etc.
...
#### User change log {/* #user-change-log */}
...
#### Username change log {/* #username-change-log */}

The most dramatic change to a user profile is when a user [changes their username](../users/user-name-change.mdx). 

All username changes are logged with the original and new usernames along with the change reason if applicable.

#### Cookie consent log {/* #cookie-consent-log */}
...
#### Moderator log and admin log {/* #moderator-log-and-admin-log */}

All [moderator](../access-privileges/staff.mdx#moderators)-specific actions, such as opening and closing or moving threads are logged in the _Moderator log_.

All [administrator](../access-privileges/staff.mdx#administrators) actions within the Admin Control Panel are logged to the _Admin log_.
...
#### Spam trigger log {/* #spam-trigger-log */}
...
#### Spam cleaner log {/* #spam-cleaner-log */}
...
#### Rejected user log {/* #rejected-user-log */}

Similar to the [spam trigger log](logs.mdx#spam-trigger-log), you may review the profiles of users whose registration was automatically rejected as being at high risk of posting spam.

### Content {/* #content */}

#### Image proxy log {/* #image-proxy-log */}

When the [image proxy](../configuration/proxy.mdx) is enabled, its use is logged in the _Image proxy log_. From here, you can see the original URL and properties of all proxied images, along with a count of how many times they have been viewed by users through the proxy.

#### Link proxy log {/* #link-proxy-log */}

As with the image proxy log, any URLs passed through the [link proxy](../configuration/proxy.mdx) are logged here together with their hits counter.

#### oEmbed log {/* #oembed-log */}

When [media is embedded in content](../content/bbcode-media-sites.mdx) and its containing HTML is fetched via [oEmbed](../content/bbcode-media-sites.mdx#oembed), statistics about the request and the number of hits on the media is recorded as part of the _oEmbed log_.

### Miscellaneous {/* #miscellaneous */}

#### Payment provider log {/* #payment-provider-log */}

Any interactions with your [configured payment providers](../configuration/payments.mdx) are recorded in the _Payment provider log_. This log can be particularly useful in determining a cause in the event that payments are failing, as a full copy of each request is stored.

#### Sitemap log {/* #sitemap-log */}

When XenForo rebuilds your [XML sitemap](../configuration/sitemap.mdx), its completion is noted in the _Sitemap log_. The log also notes across how many files the sitemap is spread, and how many URLs are recorded within the complete sitemap.