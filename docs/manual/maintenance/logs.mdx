...
#### Email bounce log
If [configured](../configuration/options-list/email.mdx) to do so, XenForo will log attempts to send email to addresses that bounce the message back due to errors such as the address not existing, the mailbox being full etc.
...
#### Rejected user log
Similar to the [spam trigger log](#spam-trigger-log), you may review the profiles of users whose registration was automatically rejected as being at high risk of posting spam.

### Content
#### Image proxy log
When the [image proxy](../configuration/options-list/proxy.mdx) is enabled, its use is logged in the _Image proxy log_. From here, you can see the original URL and properties of all proxied images, along with a count of how many times they have been viewed by users through the proxy.

#### Link proxy log
As with the image proxy log, any URLs passed through the [link proxy](../configuration/options-list/proxy.mdx) are logged here together with their hits counter.

#### oEmbed log
When [media is embedded in content](../content/embeds/bbcode-media-sites.mdx) and its containing HTML is fetched via [oEmbed](../content/embeds/bbcode-media-sites.mdx#oembed), statistics about the request and the number of hits on the media is recorded as part of the _oEmbed log_.
...
#### Sitemap log
When XenForo rebuilds your [XML sitemap](../configuration/options-list/seo.mdx#xml-sitemap-generation-and-submission), its completion is noted in the _Sitemap log_. The log also notes across how many files the sitemap is spread, and how many URLs are recorded within the complete sitemap.
