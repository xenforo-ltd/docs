...
After you have [listed users](users.md#searching-and-listing), either as a result of a search or by selecting _List all users_, you may click on any user to access the administrator's view of their user profile data. 
...
:::note
If you edit a user who is an [administrator](staff.md#administrators), you will be prompted to enter your own password to confirm any changes to their user profile.
:::
...
The user's primary [user group](groups-permissions.md) and secondary user group memberships are displayed and editable here. Users must belong to one primary user group, and may belong to any number of secondary groups.

As discussed in the [user groups](groups-permissions.md) section of this manual, we do not advise changing the primary user group to anything other than `Registered` here.

#### Display user as staff

An option is available to include the user in any staff lists published on your forum.

Note that this option is for display purposes only, and does not grant any additional permissions, rights or roles to the user. This is instead handled through the [moderator](staff.md#moderators) and [administrator](staff.md#administrators) systems.
...
All other states are self-explanatory, but it should be noted that all states other than _Valid_ will _effectively_ apply the permissions granted by the `Unregistered / unconfirmed` [user group](groups-permissions.md), rather than the standard `Registered` group.
...
The total number of messages posted, [trophy points](user-trophies.md) awarded and the total [reaction score](reactions.md) for the user are displayed and editable here.
...
This section displays any active [user upgrades](user-upgrades.md), and any [connected accounts](connected-accounts.md) in use by the current user.
...
The change log section shows the [user change log](logs.md#user-change-log) filtered to show only changed logged against this user.

## Permissions

The permissions section allows you to set [permissions](permissions.md) that apply **solely** to this user.
