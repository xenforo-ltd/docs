...
## Resource prefixes
Resource prefixes function identically to thread prefixes. This can be used to allow additional organization without creating more categories. More details are available within the [thread prefixes](../forums/threads/thread-prefixes.mdx) section.

## Resource fields
Custom resource fields allow you to define additional structured fields for users to fill in when creating a thread. These are similar to [custom thread fields](../forums/threads/thread-fields.mdx).
