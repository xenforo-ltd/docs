...
| `composer_autoload` | If your add-on uses Composer to manage dependencies, this property should contain the relative path (from the add-on root) to the directory containing the Composer vendor files (specifically where autoload_psr4.php, etc., are located). For example: vendor/composer/. XenForo will then automatically register these with its own autoloader when the add-on is active. |
