...
## Create the add-on {/* #create-the-add-on */}
...
As we're not superceding a XenForo 1 addon, we can disregard `legacy_addon_id`. For a full explanation of all of the properties in the `addon.json` file, refer to the [Add-on structure section](add-on-structure.mdx#addonjson-file).

## Creating the Setup class {/* #creating-the-setup-class */}
...
Let's start by importing some useful Schema classes. If you want to learn more about them, you can refer to the [Managing the Schema section](managing-the-schema.mdx).
...
## Extending the forum entity {/* #extending-the-forum-entity */}
...
:::note
The following steps will require [Development mode](development-tools.mdx#enabling-development-mode) to be enabled. Remember to set `Demo/Portal` as the `defaultAddOn` value in `config.php`.
...
## Extending the thread entity {/* #extending-the-thread-entity */}
...
See [Relations](entities-finders-repositories.mdx#relations) for more information about relations. Hit "Save" to save the listener.
...
## Modifying the forum edit form {/* #modifying-the-forum-edit-form */}
...
## Extending the forum save process {/* #extending-the-forum-save-process */}
...
## Setting a thread to be featured automatically {/* #setting-a-thread-to-be-featured-automatically */}
...
## Create the portal page {/* #create-the-portal-page */}
...
## Implementing the navigation tab {/* #implementing-the-navigation-tab */}
...
## Manually featuring (or unfeaturing) threads {/* #manually-featuring-or-unfeaturing-threads */}
...
## Improving the portal page {/* #improving-the-portal-page */}
...
## Implementing permissions & optimizations {/* #implementing-permissions--optimizations */}
...
## Creating some options {/* #creating-some-options */}
...
## Unfeaturing on visibility changes {/* #unfeaturing-on-visibility-changes */}

To approach this, we are going to need to modify the Thread entity again but this time we'll be doing that with the `entity_post_save` event. As we mentioned in [The Entity life cycle](entities-finders-repositories.mdx#the-entity-life-cycle), the `_postSave()` method is where actions can be performed as a result of an entity being inserted or updated. Initially we will be unfeaturing a thread when that thread is no longer visible.
...
## Some final loose ends {/* #some-final-loose-ends */}
