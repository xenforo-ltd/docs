...
## Add-on IDs and add-ons path {/* #add-on-ids-and-add-ons-path */}
...
The add-on ID you choose will also become your class namespace prefix (see [Namespaces](general-concepts.mdx#namespaces) for more information).

## Recommended version string format {/* #recommended-version-string-format */}
...
Including a valid `addon.json` file is mandatory for your addon to be recognized but you can always [validate your addon.json file](development-tools.mdx#validate-your-addonjson-file).
...
##### The requirements property {/* #the-requirements-property */}
...
### hashes.json file {/* #hashesjson-file */}
...
The `_output` directory is not required for a successful installation of an add-on, and shouldn't be included when releasing the add-on. This directory is purely for development purposes and is only used if development mode is enabled (see [Enabling development mode](development-tools.mdx#enabling-development-mode)).
...
### \_no_upload directory {/* #_no_upload-directory */}
...
### \_stubs directory {/* #_stubs-directory */}
