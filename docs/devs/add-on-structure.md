...
For example, an add-on with version string of 1.7.3 release candidate 4 would have an ID of `1070354`. The stable version of XF 2.0.0 had an ID of `2000070` while version 1.5.0 Beta 3 had an ID of `1050033`. Stable version 99.99.99 would have an ID of `99999970`... and maybe you should slow down a bit :wink:
...
| `composer_autoload` | If your add-on uses [Composer](https://getcomposer.org/) to manage dependencies, this property should contain the relative path (from the add-on root) to the directory containing the Composer vendor files (specifically where autoload_psr4.php, etc., are located). XenForo will then automatically register these with its own autoloader when the add-on is active. |
